...
$app->get('/sendPushTest/{reser}/{token}/',function (Request $request, Response $response, array $args) {
    $reser = $args['reser'];
    $token=$args['token'];
    try{
...
$app->get('/getServicesLocation/{location}', function (Request $request, Response $response, array $args) {
  $id = $args['location'];
  $sql="SELECT LS.LS_ID,LS.LOCATION_ID,SE.SERVICE_ID,SE.SERVICE_NAME,SE.DESCRIPTION,LS.PRICE,LO.TAX,LS.STATUS FROM LOCATION_SERVICE LS INNER JOIN SERVICE SE ON LS.SERVICE_ID=SE.SERVICE_ID INNER JOIN LOCATION LO ON LO.LOCATION_ID=LS.LOCATION_ID WHERE LS.STATUS=1 AND LO.LOCATION_ID=".$id." ORDER BY ORDER_ID";
...
$app->get('/getServices/{opc}', function (Request $request, Response $response, array $args) {
  $opc = $args['opc'];
  if($opc==1||$opc==2){
    $sql="SELECT * FROM SERVICE WHERE SERVICE_TYPE=".$opc;
  }else{
...
//  echo $request->getParsedBody();
  ...
  try {
    ...
    if(count($resultado)<=0){
      //print_r("1");
      $sql="SELECT SL.ID,LOCATION_ID, SC.HOUR_12,SC.HOUR FROM SCHEDULE_LOCATION SL INNER JOIN SCHEDULE SC ON SC.SCHEDULE_ID=SL.SCHEDULE_ID  WHERE SL.MAX>0 AND LOCATION_ID=".$location." ORDER BY SL.SCHEDULE_ID";
      $gsent = $db->prepare($sql);
      $gsent->execute();
      $resultado = $gsent->fetchAll(PDO::FETCH_OBJ);
      header("Access-Control-Allow-Origin: *");
      ...
      $sql="SELECT SL.ID AS ID,SL.LOCATION_ID,SCH.HOUR_12,SCH.HOUR FROM SCHEDULE_LOCATION SL INNER JOIN SCHEDULE SCH ON SCH.SCHEDULE_ID=SL.SCHEDULE_ID WHERE LOCATION_ID=".$location." AND SL.MAX>0 AND ID NOT IN (SELECT SCHEDULE_LOCATION FROM RESERVATION WHERE DATE='".$date."' AND LOCATION_ID=".$location.") ORDER BY SL.SCHEDULE_ID";
      $gsent = $db->prepare($sql);
      $gsent->execute();
      $resultado1 = $gsent->fetchAll(PDO::FETCH_OBJ);

      $sql="SELECT SC.ID,SC.LOCATION_ID,SCH.HOUR_12,SCH.HOUR FROM RESERVATION RE  INNER JOIN SCHEDULE_LOCATION SC ON RE.SCHEDULE_LOCATION=SC.ID INNER JOIN SCHEDULE SCH ON SCH.SCHEDULE_ID=SC.SCHEDULE_ID where  RE.DATE='".$date."' AND RE.LOCATION_ID=".$location." GROUP BY RE.SCHEDULE_LOCATION HAVING COUNT(*)<(SELECT MAX FROM SCHEDULE_LOCATION WHERE ID=RE.SCHEDULE_LOCATION)  ORDER BY SCH.SCHEDULE_ID";
      $gsent = $db->prepare($sql);
      $gsent->execute();
...
//  echo $request->getParsedBody();
  ...
  $sql.=" INNER JOIN SCHEDULE SCHE ON SCHE.SCHEDULE_ID=SCHLO.SCHEDULE_ID WHERE RES.CLIENT_ID='".$user."' ORDER BY RES.DATE DESC";
...
//  $sql = "INSERT INTO RESERVATION (, grapes, country, region, year, description) VALUES (:name, :grapes, :country, :region, :year, :description)";
  try {
      ...
      $sql="SELECT TRANSACCION,CLIENT_ID FROM RESERVATION WHERE RESERVATION_ID='".$dat['reservation']."'";
      $gsent = $db->prepare($sql);
      $gsent->execute();
      $exist = $gsent->fetchAll(PDO::FETCH_OBJ);
...
$app->get('/getReservations/{location}/{retailer}', function (Request $request, Response $response, array $args) {
  ...
  $sql.="PE.PHONE,CM.MAKE_NAME,CMO.MODEL_NAME,VH.YEAR,VH.COLOR,VH.PLATE,RE.EMPLOYEE,RE.LAT,RE.LNG,RE.ADDRESS,RE.SERVICE_TYPE FROM RESERVATION RE INNER JOIN LOCATION LO ON LO.LOCATION_ID=RE.LOCATION_ID INNER JOIN ";
  $sql.=" SCHEDULE_LOCATION  SL ON SL.ID=RE.SCHEDULE_LOCATION INNER JOIN SCHEDULE SCH ON ";
  $sql.=" SCH.SCHEDULE_ID=SL.SCHEDULE_ID INNER JOIN USER US ON US.USER_ID=RE.CLIENT_ID INNER JOIN PERSON PE ON PE.PERSON_ID=US.PERSON_ID  INNER ";
  $sql.="JOIN VEHICLE_DETAIL VH ON VH.VHE_DETAIL=RE.VEHICLE_DETAIL INNER JOIN CAR_MAKE CM ON ";
  $sql.="CM.MAKE_ID=VH.MAKE INNER JOIN CAR_MODEL CMO ON CMO.MODEL_ID=VH.MODEL WHERE LO.LOCATION_ID=".$id." AND LO.RETAILER_ID=".$ret;
...
$app->get('/validPromo/{code}/{user}', function (Request $request, Response $response, array $args) {
    ...
    $y = $fecha['year'];
    $date=$y."-".$m."-".$d;
    $sql="SELECT ID,DISCOUNT FROM PROMO WHERE CODE='".$code."' AND START<='".$date."' AND FINISH>='".$date."'";
    error_log($sql);
  try {
